#!/usr/bin/env python3
"""
Traceroute implementation for network path analysis
Similar to Zenmap's traceroute functionality
"""
...
class TracerouteHop:
    ...
    def _determine_status(self) -> str:
        """Determine hop status based on available data"""
        if self.ip is None:
            return "timeout"
...
class TracerouteResult:
    ...
    def __init__(self, target: str, max_hops: int = 30):
        self.target = target
        self.target_ip = None  # Will be set by traceroute functions
        self.max_hops = max_hops
        self.method = "unknown"  # Will be set by traceroute functions
    ...
    def get_final_hop(self) -> Optional[TracerouteHop]:
        """Get the final hop (destination)"""
        if not self.hops:
            return None
        
        # Find last hop with IP
    ...
    def reached_destination(self) -> bool:
        """Check if traceroute reached the target"""
        final_hop = self.get_final_hop()
        if not final_hop:
            return False
        
        try:
            # Resolve target to IP for comparison
...
def traceroute_system(target: str, max_hops: int = 30, timeout: int = 5) -> TracerouteResult:
    """
    Perform traceroute using system command (most reliable)
    """
...
def traceroute_icmp(target: str, max_hops: int = 30, timeout: int = 5) -> TracerouteResult:
    """
    Perform traceroute using raw ICMP sockets (requires root)
    """
    result = TracerouteResult(target, max_hops)
    result.method = "icmp"
    ...
    try:
        ...
        if not result.success:
            result.finish(True)  # Completed even if didn't reach target
...
def traceroute_udp(target: str, max_hops: int = 30, timeout: int = 5, 
    ...
    """
    Perform traceroute using UDP probes (Unix-style traceroute)
    """
    result = TracerouteResult(target, max_hops)
    result.method = "udp"
...
def _send_icmp_probe(icmp_socket, target_ip: str, ttl: int, timeout: int) -> TracerouteHop:
    """Send ICMP probe with specific TTL"""
    ...
    try:
        ...
        if valid_ips:
            hop.ip = valid_ips[0]  # Use first valid IP
            try:
...
def _send_udp_probes(target_ip: str, ttl: int, timeout: int, port: int) -> TracerouteHop:
    """Send UDP probes with specific TTL"""
    hop = TracerouteHop(ttl)
    ...
    try:
        ...
        # Create ICMP socket to receive responses
        icmp_socket = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP)
        icmp_socket.settimeout(timeout)
...
def _extract_ip_hostname(host_str: str) -> Tuple[Optional[str], Optional[str]]:
    """Extract IP and hostname from host string"""
...
def traceroute(target: str, max_hops: int = 30, timeout: int = 5, method: str = "auto") -> TracerouteResult:
    ...
    if method == "system":
        ...
        # Try system first, fall back to others
        result = traceroute_system(target, max_hops, timeout)
        if result.success:
            return result
        
        # Try ICMP if we have root
        try:
            return traceroute_icmp(target, max_hops, timeout)
        except:
            pass
...
def trace(target: str, **kwargs) -> TracerouteResult:
    """Convenience function for traceroute"""
